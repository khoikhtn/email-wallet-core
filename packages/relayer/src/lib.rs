...
static CHAIN_SDK_PROXY_SERVER: OnceLock<String> = OnceLock::new();

pub async fn setup() -> Result<()> {
    dotenv().ok();
    PRIVATE_KEY.set(env::var(PRIVATE_KEY_KEY).unwrap()).unwrap();
    CHAIN_SDK_PROXY_SERVER.set(env::var("CHAIN_SDK_PROXY_SERVER").unwrap()).unwrap();
    // CHAIN_ID
...
pub async fn run(config: RelayerConfig) -> Result<()> {
    ...
    //     .unwrap();
    CHAIN_SDK_PROXY_SERVER.set(var("CHAIN_SDK_PROXY_SERVER").unwrap()).unwrap();
    ...
    let client = Arc::new(ChainClient::setup().await.unwrap());
