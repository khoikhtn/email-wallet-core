...
impl SendCommandAsset {
    pub fn from_email_subject(subject: &str) -> Result<Self> {
        let words = subject.split_whitespace().collect::<Vec<&str>>();
        ensure!(words[0] == "Send", "Send command must start with 'Send'");
        ensure!(words.last().unwrap().starts_with("xion"), "Invalid xion address");
        ensure!(words[words.len() - 2] == "to", "Send command must have 'to'");

        match words[1] {
            // Send CW20 amount contract_address to xion_address
            "CW20" => Ok(Self::CW20 {
                to_address: words.last().unwrap().to_string(),
                contract_address: words[3].to_string(),
                amount: words[2].parse()?,
            }),
            // Send NFT contract_address token_id to xion_address
            "NFT" => Ok(Self::NFT {
                to_address: words.last().unwrap().to_string(),
                contract_address: words[2].to_string(),
                token_id: words[3].to_string(),
            }),
            // Send amount denom to xion_address
            _ => Ok(Self::NativeToken {
                to_address: words.last().unwrap().to_string(),
                denom: words[2].to_string(),
                amount: words[1].parse()?,
            })
        }
    }
}